...
function extractFinishReason(response) {
  const candidate = response?.response?.candidates?.[0] || response?.candidates?.[0] || null;
  return String(candidate?.finishReason || candidate?.finish_reason || "").trim();
}
  ...
  for (const modelName of uniqueModels()) {
    for (let attempt = 0; attempt <= retries; attempt += 1) {
      try {
        ...
        return { model: modelName, text, finishReason: extractFinishReason(result), raw: result?.response || null };
      } catch (error) {
        lastError = error;
