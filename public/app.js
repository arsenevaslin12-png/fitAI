...
/api/workout?config=1 peut renvoyer {supabaseUrl, supabaseAnonKey}
ou {SUPABASE_URL, SUPABASE_ANON_KEY}. On accepte les 2.
*/
...
const j = await r.json().catch(() => ({}));
  ...
  // - ancien: SUPABASE_URL / SUPABASE_ANON_KEY
  // - actuel: supabaseUrl / supabaseAnonKey
  ...
  if (!url || !anon) {
    ...
    this.toast(
      "Config Supabase manquante (API /api/workout?config=1). Vérifie tes variables sur Vercel + le code API.",
      "danger"
    );
    return;
  }

  this.cfg = { ...j, SUPABASE_URL: url, SUPABASE_ANON_KEY: anon };
