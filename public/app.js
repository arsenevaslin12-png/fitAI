...
function toast(message, cls = "ok") {
  const isNetwork = /réseau|network|cors|connexion/i.test(String(message));
  if (isNetwork && Date.now() - NETWORK_TOAST_AT < 4000) return;
  if (isNetwork) NETWORK_TOAST_AT = Date.now();
