...
function looksLikeIncompleteReply(text, finishReason = "") {
  const clean = cleanConversationText(text);
  if (!clean) return false;
  if (/\bMAX_TOKENS\b/i.test(String(finishReason || ""))) return true;
  const compact = clean.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
  if (!compact) return false;
  const lastLine = compact.split(/\n+/).filter(Boolean).pop() || compact;
  if (lastLine.length < 18) return false;
  if (/[.!?…»"')\]]$/.test(lastLine)) return false;
  if (/:$/.test(lastLine)) return true;
  return /[A-Za-zÀ-ÖØ-öø-ÿ0-9]$/.test(lastLine);
}

async function extendIncompleteConversationReply(apiKey, prompt, partialText) {
  const continuationPrompt = `${prompt}

Réponse déjà générée (potentiellement coupée):
${partialText}

Consigne finale: complète uniquement la fin de cette réponse en 1 ou 2 phrases maximum, sans répéter le début, avec une vraie ponctuation finale.`;
  const continuation = await generateWithRetry(apiKey, continuationPrompt, {
    temperature: 0.25,
    maxOutputTokens: 180,
    timeoutMs: 5000,
    retries: 0
  });
  return cleanConversationText(continuation.text || "");
}
...
async function generateConversationReply({ apiKey, intent, message, history, profile, goalContext }) {
  ...
  try {
    const result = await generateWithRetry(apiKey, prompt, { temperature: 0.55, maxOutputTokens: 1400, timeoutMs: 10000, retries: 1 });
    let text = cleanConversationText(result.text || "");
    if (!text) throw new Error("EMPTY_CONVERSATION");

    if (looksLikeIncompleteReply(text, result.finishReason)) {
      try {
        const extra = await extendIncompleteConversationReply(apiKey, prompt, text);
        if (extra) {
          const normalizedExtra = extra.replace(/^[-–•\s]+/, "").trim();
          if (normalizedExtra && !text.includes(normalizedExtra)) {
            text = `${text}${/\s$/.test(text) ? "" : " "}${normalizedExtra}`.trim();
          }
        }
      } catch (continuationError) {
        console.warn("[coach][conversation][continuation_failed]", String(continuationError?.message || continuationError).slice(0, 160));
      }
    }

    if (looksLikeIncompleteReply(text)) text = `${text}.`;
