...
function stripCoachMarkdown(text = "") {
  return String(text || "")
    .replace(/^```[\w-]*\s*/g, "")
    .replace(/```$/g, "")
    .replace(/\u00a0/g, " ")
...
function hasTerminalCoachPunctuation(text = "") {
  return /[.!?…]["')\]]*\s*$/.test(String(text || "").trim());
}

function looksLikeCoachReplyCut(text = "") {
  const s = stripCoachMarkdown(text);
  if (!s) return true;
  if (hasTerminalCoachPunctuation(s)) return false;
  const tail = s.split(/\n+/).pop().trim();
  if (!tail) return false;
  if (tail.split(/\s+/).length <= 4) return true;
  if (/\b(et|ou|mais|car|donc|pour|avec|sans|sur|dans|vers|de|du|des|en|au|aux|a|se|te|me|je|tu|il|elle|nous|vous|ils|elles|on|ta|ton|tes|mon|ma|mes|sa|son|ses|leur|leurs|un|une|le|la|les|lorsque|quand)\s*$/i.test(tail)) return true;
  return true;
}

function buildCoachReplyClosure(intent, message, profile = {}, goalContext = {}) {
  const p = makeProfileSummary(profile, goalContext);
  const goal = getGoalDescription(p.goal);
  const level = getLevelDescription(p.level);
  if (intent === "motivation_question") return "Fais la version la plus simple possible aujourd'hui, mais fais-la vraiment.";
  if (intent === "recovery_question") return "Priorité récupération utile aujourd'hui : calme le jeu, dors mieux et repars plus propre demain.";
  if (intent === "nutrition_question") return "Reste simple : protéines, légumes, eau et une structure que tu peux réellement tenir aujourd'hui.";
  if (intent === "progress_question") return "Le plus rentable maintenant : une seule variable à la fois, puis 7 jours de régularité avant de réévaluer.";
  return `Reste simple et régulier : ${goal}, niveau ${level}.`;
}

async function finalizeCoachReply({ apiKey, prompt, draft, intent, message, history, profile, goalContext }) {
  let text = stripCoachMarkdown(draft);
  if (!text) throw new Error("EMPTY_CONVERSATION");
  if (!looksLikeCoachReplyCut(text)) return text;

  if (apiKey) {
    try {
      const completionPrompt = `Tu complètes uniquement la FIN d'une réponse de coach déjà commencée.

Règles:
- N'écris pas le début.
- Ajoute seulement la fin manquante.
- 1 à 2 phrases maximum.
- Pas de liste.
- Pas de salutation.
- Termine avec une ponctuation finale claire.

Réponse à compléter:
"""${text}"""`;
      const result = await generateWithRetry(apiKey, completionPrompt, { temperature: 0.2, maxOutputTokens: 180, timeoutMs: 4500, retries: 0 });
      const suffix = stripCoachMarkdown(result.text || "");
      if (suffix) {
        const glue = /^[,.;:!?)]/.test(suffix) || /[\s"'(]$/.test(text) ? "" : " ";
        const merged = stripCoachMarkdown(text + glue + suffix).replace(/\s{2,}/g, " ").trim();
        if (hasTerminalCoachPunctuation(merged)) return merged;
        text = merged;
      }
    } catch {}
  }
  ...
  if (!text) text = fallbackConversation(intent, message, profile, goalContext);
  if (!hasTerminalCoachPunctuation(text)) {
    const closure = buildCoachReplyClosure(intent, message, profile, goalContext);
    text = `${text}. ${closure}`.replace(/\.\s*\./g, ".").trim();
  }
  return text;
}
...
async function generateConversationReply({ apiKey, intent, message, history, profile, goalContext }) {
  ...
  try {
    const result = await generateWithRetry(apiKey, prompt, { temperature: 0.55, maxOutputTokens: 2400, timeoutMs: 14000, retries: 1 });
    const text = await finalizeCoachReply({ apiKey, prompt, draft: result.text, intent, message, history, profile, goalContext });
    if (!text) throw new Error("EMPTY_CONVERSATION");
